...
- 性別が「回答しない」の場合、当該年齢区分の男女基準値のうち大きい方（より多くの摂取を推奨する方）を、9項目それぞれ独立に（項目ごとに）採用する（per-nutrient max。栄養素によって男女どちらが大きいかが異なりうるため、行単位でどちらかの性別の行をまるごと選ぶのではなく項目ごとに判定する）
- 例外: `saltEquivalentUpperLimitG`（食塩相当量の上限目安）は他8項目と異なり「目標量（未満）」という上限フィールドであり、値が大きいほど「より多く摂ってよい＝より緩い」方向になる。「大きい方を採用する」という上記ルールをこのフィールドにそのまま適用すると、「回答しない」利用者に最も緩い（保護的でない）上限を与えてしまい、本ルールの趣旨（過小な目標設定＝利用者にとって不利な方向を避けること）と矛盾する結果になる。そのため `saltEquivalentUpperLimitG` に限り、男女基準値のうち小さい方（より厳しい、より保護的な上限）を採用する（2026-08-27 remediation: 実装時のレビューで指摘され、ユーザーが明示的に承認した設計上の例外）
